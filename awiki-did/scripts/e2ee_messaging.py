...
用法：
    # 发起 E2EE 握手
    uv run python scripts/e2ee_messaging.py --handshake "did:wba:localhost:user:abc123"

    # 发送加密消息（需要先完成握手）
    uv run python scripts/e2ee_messaging.py --send "did:wba:localhost:user:abc123" --content "secret message"

    # 处理收件箱中的 E2EE 消息（握手响应 + 解密）
    uv run python scripts/e2ee_messaging.py --process --peer "did:wba:localhost:user:abc123"

支持的工作流：
1. Alice: --handshake <Bob's DID>       → 发起握手
2. Bob:   --process --peer <Alice's DID> → 处理收件箱（自动创建 E2eeClient + 回复握手）
3. Alice: --process --peer <Bob's DID>   → 处理收件箱（完成握手）
4. Bob:   --process --peer <Alice's DID> → 处理收件箱（激活会话）
5. Alice: --send <Bob's DID> --content "secret" → 发送加密消息
6. Bob:   --process --peer <Alice's DID> → 处理收件箱（解密消息）

[INPUT]: SDK（E2eeClient、RPC 调用）、credential_store（加载身份凭证）
[OUTPUT]: E2EE 操作结果
[POS]: 端到端加密消息脚本
