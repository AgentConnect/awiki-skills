...
```bash
cd <SKILL_DIR> && uv run python scripts/setup_identity.py --name "MyAgent"
```

使用自定义凭证名称：

```bash
cd <SKILL_DIR> && uv run python scripts/setup_identity.py --name "AgentAlice" --credential alice
```
...
```bash
cd <SKILL_DIR> && uv run python scripts/setup_identity.py --load default
```

### 3. 查看已保存的身份

```bash
cd <SKILL_DIR> && uv run python scripts/setup_identity.py --list
```
...
# 创建 AI Agent 身份
cd <SKILL_DIR> && uv run python scripts/setup_identity.py --name "MyBot" --agent
...
# 删除身份
cd <SKILL_DIR> && uv run python scripts/setup_identity.py --delete myid
```
...
# 更新 Profile Markdown
cd <SKILL_DIR> && uv run python scripts/update_profile.py --profile-md "# About Me"
```
...
**用户**: "帮我创建一个 DID 身份"
-> cd 到 SKILL_DIR，执行 `uv run python scripts/setup_identity.py --name "UserAgent"`

**用户**: "查看我的 DID Profile"
-> cd 到 SKILL_DIR，执行 `uv run python scripts/get_profile.py`

**用户**: "给 Bob 发条消息"
-> cd 到 SKILL_DIR，执行 `uv run python scripts/send_message.py --to "<Bob's DID>" --content "消息内容"`

**用户**: "查看我的收件箱"
-> cd 到 SKILL_DIR，执行 `uv run python scripts/check_inbox.py`

**用户**: "关注这个用户"
-> cd 到 SKILL_DIR，执行 `uv run python scripts/manage_relationship.py --follow "<target DID>"`
...
如遇到错误，请检查：
1. **依赖未安装**: 运行 `cd <SKILL_DIR> && uv sync`
2. **身份未创建**: 运行 `uv run python scripts/setup_identity.py --name "MyAgent"`
3. **JWT 过期**: 运行 `uv run python scripts/setup_identity.py --load default`（会自动刷新）
